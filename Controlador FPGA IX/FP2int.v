...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone IV E" OPERATION="FLOAT2INT" ROUNDING="TO_NEAREST" WIDTH_DATA=64 WIDTH_EXP_INPUT=11 WIDTH_EXP_OUTPUT=8 WIDTH_INT=27 WIDTH_MAN_INPUT=52 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=27 aclr clk_en clock dataa nan overflow result underflow
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone IV E" PIPELINE=2 SHIFTDIR="LEFT" SHIFTTYPE="LOGICAL" WIDTH=78 WIDTHDIST=7 aclr clk_en clock data distance result
//VERSION_BEGIN 13.0 cbx_altbarrel_shift 2013:06:12:18:03:43:SJ cbx_mgl 2013:06:12:18:05:10:SJ  VERSION_END

//synthesis_resources = reg 161 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  FP2int_altbarrel_shift_90g
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[77:0]	sbit_piper1d;
	reg	[77:0]	sbit_piper2d;
	reg	sel_pipec4r1d;
	reg	sel_pipec5r1d;
	reg	sel_pipec6r1d;
	wire  [7:0]  dir_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 78'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[311:234];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 78'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[545:468];
	// synopsys translate_off
	initial
		sel_pipec4r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		direction_w = 1'b0,
		pad_w = {64{1'b0}},
		result = sbit_w[623:546],
		sbit_w = {sbit_piper2d, smux_w[467:312], sbit_piper1d, smux_w[233:0], data},
		sel_w = {sel_pipec6r1d, sel_pipec5r1d, sel_pipec4r1d, distance[3:0]},
		smux_w = {((({78{(sel_w[6] & (~ dir_w[6]))}} & {sbit_w[481:468], pad_w[63:0]}) | ({78{(sel_w[6] & dir_w[6])}} & {pad_w[63:0], sbit_w[545:532]})) | ({78{(~ sel_w[6])}} & sbit_w[545:468])), ((({78{(sel_w[5] & (~ dir_w[5]))}} & {sbit_w[435:390], pad_w[31:0]}) | ({78{(sel_w[5] & dir_w[5])}} & {pad_w[31:0], sbit_w[467:422]})) | ({78{(~ sel_w[5])}} & sbit_w[467:390])), ((({78{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[373:312], pad_w[15:0]}) | ({78{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[389:328]})) | ({78{(~ sel_w[4])}} & sbit_w[389:312])), ((({78{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[303:234], pad_w[7:0]}) | ({78{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[311:242]})) | ({78{(~ sel_w[3])}} & sbit_w[311:234])), ((({78{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[229:156], pad_w[3:0]}) | ({78{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[233:160]})) | ({78{(~ sel_w[2])}} & sbit_w[233:156])), ((({78{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[153:78], pad_w[1:0]}) | ({78{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[155:80]})) | ({78{(~ sel_w[1])}} & sbit_w[155:78])), ((({78{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[76:0], pad_w[0]}) | ({78{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[77:1]})) | ({78{(~ sel_w[0])}} & sbit_w[77:0]))};
endmodule //FP2int_altbarrel_shift_90g

//synthesis_resources = lpm_add_sub 5 lpm_compare 4 reg 384 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  FP2int_altfp_convert_3dq
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	wire  [77:0]   wire_altbarrel_shift6_result;
	...
	reg	int_or_reg2;
	reg	int_or_reg3;
	reg	[26:0]	integer_result_reg;
	reg	[25:0]	integer_rounded_reg;
	...
	wire  [25:0]   wire_add_sub7_result;
	...
	wire  add_1_cout_w;
	wire  add_1_w;
	wire  [26:0]  all_zeroes_w;
	wire  [77:0]  barrel_mantissa_input;
	wire  [24:0]  barrel_zero_padding_w;
	wire  below_limit_exceeders;
	wire  [26:0]  below_limit_integer;
	...
	wire  [26:0]  exceed_limit_integer;
	...
	wire  [26:0]  infinity_value_w;
	wire  int_or1_w;
	wire  [26:0]  integer_output;
	wire  [25:0]  integer_post_round;
	wire  [25:0]  integer_pre_round;
	wire  [26:0]  integer_result;
	wire  [25:0]  integer_rounded;
	wire  [25:0]  integer_rounded_tmp;
	wire  [25:0]  integer_tmp_output;
	wire  [12:0]  inv_add_1_adder1_w;
	wire  [12:0]  inv_add_1_adder2_w;
	wire  [25:0]  inv_integer;
	wire  [77:0]  lbarrel_shift_result_w;
	wire  [25:0]  lbarrel_shift_w;
	wire  lower_limit_selector;
	wire  lowest_integer_selector;
	wire  [25:0]  lowest_integer_value;
	wire  [25:0]  man_bus1;
	wire  [25:0]  man_bus2;
	wire  [25:0]  man_or1;
	wire  man_or1_w;
	...
	wire  nan_input_w;
	wire  [26:0]  neg_infi_w;
	wire  [10:0]  padded_exponent_input;
	wire  [26:0]  pos_infi_w;
	wire  [6:0]  power2_value_w;
	wire  [26:0]  result_w;
	wire  round_bit_w;
	wire  sign_input;
	wire  sign_input_w;
	wire  [25:0]  signed_integer;
	wire  sticky_bit_w;
	wire  [50:0]  sticky_bus;
	wire  [50:0]  sticky_or;
	wire  [25:0]  unsigned_integer;
	wire  upper_limit_w;
	wire  zero_input_w;

	FP2int_altbarrel_shift_90g   altbarrel_shift6
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_result_reg <= 27'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_rounded_reg <= 26'b0;
	...
	.datab(26'b00000000000000000000000001),
	.overflow(),
	...
	assign
		...
		all_zeroes_w = {1'b0, {26{1'b0}}},
		barrel_mantissa_input = {barrel_zero_padding_w, implied_mantissa_input},
		barrel_zero_padding_w = {25{1'b0}},
		below_limit_exceeders = (((denormal_input_w | zero_input_w) | lower_limit_selector) | nan_input_w),
		below_limit_integer = (({27{(~ below_limit_exceeders)}} & integer_output) | ({27{below_limit_exceeders}} & all_zeroes_w)),
		...
		const_bias_value_add_width_res_w = 11'b10000011001,
		...
		exceed_limit_integer = (({27{(~ exceed_limit_exceeders)}} & below_limit_integer) | ({27{exceed_limit_exceeders}} & infinity_value_w)),
		...
		infinity_value_w = (({27{(~ sign_input_w)}} & pos_infi_w) | ({27{sign_input_w}} & neg_infi_w)),
		int_or1_w = man_or2[0],
		...
		integer_rounded = (({26{(~ lowest_integer_selector)}} & integer_rounded_tmp) | ({26{lowest_integer_selector}} & lowest_integer_value)),
		integer_rounded_tmp = (({26{(~ add_1_w)}} & integer_pre_round) | ({26{add_1_w}} & integer_post_round)),
		integer_tmp_output = (({26{(~ sign_input_w)}} & unsigned_integer) | ({26{sign_input_w}} & signed_integer)),
		inv_add_1_adder1_w = wire_add_sub8_result,
		inv_add_1_adder2_w = (({13{(~ wire_add_sub8_cout)}} & inv_integer[25:13]) | ({13{wire_add_sub8_cout}} & wire_add_sub9_result)),
		inv_integer = (~ integer_rounded_reg),
		lbarrel_shift_result_w = wire_altbarrel_shift6_result,
		lbarrel_shift_w = lbarrel_shift_result_w[77:52],
...
endmodule //FP2int_altfp_convert_3dq
//VALID FILE
...
module FP2int (
	...
	wire  nan = sub_wire2;
	wire [26:0] result = sub_wire3[26:0];

	FP2int_altfp_convert_3dq	FP2int_altfp_convert_3dq_component (
				.aclr (aclr),
				.clk_en (clk_en),
				.clock (clock),
				.dataa (dataa),
